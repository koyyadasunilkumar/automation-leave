package ai.hono.devnt8.util.unpaidleave.test;
...
@Epic("Hono Leave")
@Feature("Cancel Unpaid Leave request after two level approval test")
public class CreateUnpaidLeaveRequestTest extends BaseHonoLeaveTest {
	...
	private static final Logger logger = Logger
			.getLogger(CreateUnpaidLeaveRequestTest.class.getName());
	...
	public void initMethodTest(String browser, String siteURL, String userName, String approverPassword)
		...
		driver = this.getWebDriver(browser, WEB_DRIVER.CREATE_UNPAID_LEAVE_REQUEST);
	...
	public void verifyCreateLeaveRequestByGivingAllValidDetails() {
		...
		try {
			...
			createLeaveRequestPage.selectLeaveType(testDataProp.getProperty(UNPAID_LEAVE_TYPE_TEXT));
			...
			createLeaveRequestPage.setLeaveReason(testDataProp.getProperty(UNPAID_LEAVE_REASON_TEXT));
	...
	public void verifyApproveLeaveRequestByWithoutEnteringReason()throws Exception {
		...
		try {
			...
			Assert.fail("Exception occured while testing verifyApproveLeaveRequestByWithoutEnteringReason : "
					+ e.getMessage());
