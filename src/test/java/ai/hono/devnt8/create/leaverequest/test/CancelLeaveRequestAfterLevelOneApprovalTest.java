...
@Feature("Approve leave request After level one approval test")
public class CancelLeaveRequestAfterLevelOneApprovalTest extends BaseHonoLeaveTest {
	...
	private static final Logger logger = Logger.getLogger(CancelLeaveRequestBeforeApprovalTest.class.getName());
	...
	public void verifyDashboardDetailsAfterLoginWithValidDetails() {
		...
		try {
			...
			Thread.sleep(3000);
			Assert.assertEquals(loginPage.getDashboardText(), expectedAssertionsProp.getProperty(DASHBOARD_TEXT));
