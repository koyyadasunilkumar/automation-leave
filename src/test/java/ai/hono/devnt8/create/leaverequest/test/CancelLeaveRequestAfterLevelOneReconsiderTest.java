...
import io.qameta.allure.Epic;
	...
	import io.restassured.response.Response;
		...
		@Feature("Cancel leave request After level one reconsider test")
		public class CancelLeaveRequestAfterLevelOneReconsiderTest extends BaseHonoLeaveTest {
			...
			private static final Logger logger = Logger.getLogger(CancelLeaveRequestAfterLevelOneApprovalTest.class.getName());
			...
			public void verifyCreateLeaveRequestByGivingAllValidDetails() {
				...
				try {
					...
					createLeaveRequestPage.selectLeaveType(testDataProp.getProperty(LEAVE_TYPE_SICK_LEAVE_TEXT));
			...
			public void verifyLeaveRequestBySelectCancelOption() {
				...
				try {
					...
					createLeaveRequestPage.setLeaveReason(testDataProp.getProperty(CANCELLATION_APPROVAL_REMARKS_TEXT));
					createLeaveRequestPage.clickOnCancelRequestButton();

					Assert.assertEquals(leaveTypePage.getLeaveToastMessage(),
							expectedAssertionsProp.getProperty(LEAVE_REQUEST_CANCELLED_SUCCESSFULLY_TEXT));
